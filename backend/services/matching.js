...
async function ruleFilter(userId, scene) {
  ...
  const userFields = isCommute
    ? 'u.nickname, u.avatar_url, u.about_me'
    : 'u.nickname, u.department, u.avatar_url, u.about_me';
  ...
  for (const r of pick3) {
    ...
    try {
      ...
      if (candidateProfile) {
        ...
        const timeout = new Promise(resolve => setTimeout(() => resolve(null), 35000));
        Promise.race([iceFn(myProfile, candidateProfile), timeout])
          .then(ice => (ice && ice.inviteMessage && ice.inviteMessage !== '嘿，要不要一起？' && ice.inviteMessage !== '一起呀~') ? ice : fallbackIce)
          .catch(() => fallbackIce)
          .then(ice => {
            console.log('[ICE] 写入 matchId=%d inviteMessage=%s', matchId, ice.inviteMessage);
