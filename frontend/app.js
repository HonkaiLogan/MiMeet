...
function pollIcebreaker(matchId, accentColor) {
  ...
  const timer = setInterval(async () => {
    ...
    try {
      ...
      if (res && res.ready && res.inviteMessage) {
        clearInterval(timer);
    ...
    if (attempts >= maxAttempts) {
      clearInterval(timer);
      const el2 = document.querySelector(`.ice-section[data-match-id="${matchId}"]`);
...
// ============ API 请求 ============
// Mock 数据标记：强制使用前端 mock 数据，不依赖后端
let useMockData = true;
let mockDataCheckDone = true;
...
<div id="pref-drawer">
  ...
  <div class="ice-section" data-match-id="${matchId}">
    <div class="p-3 bg-gray-50 rounded-lg mb-3 flex items-center gap-2">
      <span class="animate-spin text-sm">⏳</span>
      <span class="text-xs text-gray-400">破冰话术生成中...</span>
    </div>
  </div>
  <div class="flex space-x-3">
    ...
    <button data-uid="${uid}" data-match-id="${matchId}" class="detail-btn pressable flex-1 h-10 bg-gray-100 text-gray-600 text-sm font-medium rounded-lg">查看详情</button>
      ...
      // 异步轮询破冰话术
      mock.forEach(i => {
        if (i.match_id) pollIcebreaker(i.match_id, 'lunch');
      });
    } catch (error) {
      document.getElementById('loading')?.classList.add('hidden');
  ...
  init(params = {}) {
    ...
    document.getElementById('ep-save').addEventListener('click', async () => {
      ...
      saveUserProfile({
        ...
        interests: fullProfile.interestTags || [],
      }).catch(() => {});
      // 更新顶部标签
